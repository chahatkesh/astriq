# Engineering Docs

Internal guidance for the Birth Chart Generator codebase.

- [Architecture: Codebase Structure](architecture/codebase-structure.md)
- [Deployment: GitHub Governance](deployment/github-governance.md)
- [Deployment: Production Runtime](deployment/production-runtime.md)
- [Deployment: Root CI/CD](deployment/root-ci-cd.md)
- [Setup: Local Development](setup/local-development.md)
- [Setup: SOPS Secret Management](setup/secrets/sops-secret-management.md)
- [Testing: Quality Gates](testing/quality-gates.md)
- [Decision Records](decisions/README.md)
